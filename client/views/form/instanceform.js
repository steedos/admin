    ...
    for(var i = 0; i < stFields.length; i ++){
      ...
      if (stField.type == 'table'){
        ...
        afFields[stField.code] = {
                                    ...
                                    minCount : 0,
                                    maxCount : 200,
                                    autoform : instance.values[stField.code]
                                  };

        afFields[stField.code + ".$"] = {type:Object,optional:true};

        for(var si = 0 ; si < sFields.fields.length; si++){
         
          var sstField = sFields.fields[si];

          afFields[stField.code] = new s_schema(String, label, sstField.code, sstField.type, '', sstField.options);
        }
  ...
  Template.instanceform.helpers({
    ...
    subformValue: function(code){
      console.log(fields[code].autoform.value);
      var subV = fields[code].autoform.value;
