  ...
  function sFToAF(steedosForm,instance) {
    var afFields = {};
    ...
    for(var i = 0; i < stFields.length; i ++){
      ...
      if (stField.type == 'table'){
        ...
        afFields[stField.code] = {
                                    ...
                                    minCount : 0,
                                    maxCount : 200,
                                    autoform : {values:instance.values[stField.code]}
                                  };

        afFields[stField.code + ".$"] = {type:Object,optional:true};

        for(var si = 0 ; si < stField.sfields.length; si++){
         
          var sstField = stField.sfields[si];

          afFields[stField.code + ".$." + sstField.code] = new s_schema(String, sstField.code, sstField.code, sstField.type, '', sstField.options);
          //afFields[stField.code + ".$." + sstField.code] = {type:String};
        }

      }else{
        
        afFields[stField.code] = new s_schema(String, label, stField.code, stField.type, instance.values[stField.code]);
  ...
  function s_schema(schematype, label, fieldName, fieldType, fieldValue, options){

    schema = {type:schematype};
   
    schema.label = label;
    schema.autoform = new s_autoform(fieldName, fieldType, fieldValue, options);
    return schema;
  }

  function s_autoform(name,type,value,options){
    autoform = {};
    //autoform.name = name;
    autoform.value = value;
    //字段类型转换
    switch(type){
      case 'input' :
            autoform.type = 'text';
  ...
  Template.instanceform.helpers({
    ...
    subformValue: function(code){
      console.log(fields[code].autoform.values);
      var subV = fields[code].autoform.values;
